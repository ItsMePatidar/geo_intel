...
with open('/Users/priyanshpatidar/Documents/FPL/H3_Data_Construction/polygon.json', 'r') as file:
    data = json.load(file)
# --- Config ---
TIFF_PATH = "/Users/priyanshpatidar/Documents/FPL/H3_Data_Construction/WorldPop Urbanisation/Data/IND_DUG_2025_GRID_L2_R2025A_v1.tif"
...
print("\nWriting results to urbanisation table...")
print(df.head())
urbanisation_rows = [
...
try:
    ...
    filtered_rows = [(hex, wc) for hex, wc in urbanisation_rows if hex in existing_hexes]
    skipped = len(urbanisation_rows) - len(filtered_rows)
    if skipped:
    ...
    execute_batch(
        cur,
        """
        INSERT INTO urbanisation (hex, urbanisation_index)
        VALUES (%s, %s)
        ON CONFLICT (hex) DO UPDATE
            SET urbanisation_index = EXCLUDED.urbanisation_index
        """,
        filtered_rows,
        page_size=500
    )
    print(f"  ✓ Upserted {len(filtered_rows)} rows into urbanisation")
