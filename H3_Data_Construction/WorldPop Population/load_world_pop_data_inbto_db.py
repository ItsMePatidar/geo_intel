...
with open('/Users/priyanshpatidar/Documents/FPL/H3_Data_Construction/polygon.json', 'r') as file:
    data = json.load(file)
# --- Config ---
TIFF_PATH = "/Users/priyanshpatidar/Documents/FPL/H3_Data_Construction/WorldPop Population/Data/worldpop_india_population_100m.tif"
H3_RESOLUTION = 9  # ~174m hexagons; use 8 for ~461m

# Pune bounding box (rough)
PUNE_GEOJSON = {
    "type": "Polygon",
    "coordinates": [i[::-1] for i in data['coordinates'][0]]
}
...
print("\nWriting results to world_population table...")
print(df.head())
world_population_rows = [
...
try:
    ...
    filtered_rows = [(hex, wc) for hex, wc in world_population_rows if hex in existing_hexes]
    skipped = len(world_population_rows) - len(filtered_rows)
    if skipped:
    ...
    execute_batch(
        cur,
        """
        INSERT INTO world_population (hex, world_population_index)
        VALUES (%s, %s)
        ON CONFLICT (hex) DO UPDATE
            SET world_population_index = EXCLUDED.world_population_index
        """,
        filtered_rows,
        page_size=500
    )
    print(f"  ✓ Upserted {len(filtered_rows)} rows into world_population")
